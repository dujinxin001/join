...
def do_handle_data(context, data):
    ...
    if gr_index2 <= g.index_growth_rate and gr_index8 <= g.index_growth_rate:
        log.info("==> 指数20日涨幅小于等于判定调仓的二八指数20日增幅--")
        #清仓卖出所有股票
        ...
        # 调仓计数日%调仓频率（4%3=1）
        log.info("==> g.day_count模g.period：%s" %(g.day_count % g.period))
...
def clear_position(context):
    #log.info("准备清仓，卖出所有股票")
    if context.portfolio.positions:
        log.info("==>开始清仓，卖出所有股票")
        for stock in context.portfolio.positions.keys():
            position = context.portfolio.positions[stock]
...
def pick_stocks(context, data,gr_index2,gr_index8):
    ...
    log.info("==> 过滤后评分前股票代码：%s" %(stock_list))
    if g.is_rank_stock:
