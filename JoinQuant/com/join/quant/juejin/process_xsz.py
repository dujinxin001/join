...
import math
import com.join.quant.juejin.tradestat as tradestat
...
class process_xsz():
    ...
    def handle_data(self,context, data):
        ...
        if hour == adjust_position_time[g.HOUR] and minute == adjust_position_time[g.MINUTE] and not g.cache['stop_trade']:
            self._adjust_position(context, data)
    
    
    def _adjust_position(self,context, data):
        log.info("调仓日计数 [%d]" % (g.cache['day_count']))
    
        # 回看指数前20天的涨幅
        gr_index_l = self.get_growth_rate(g.param['index_l'][g.VALUE])
        gr_index_s = self.get_growth_rate(g.param['index_s'][g.VALUE])
        log.info("当前%s指数的20日涨幅 [%.2f%%]" % (
            self.get_security_info(g.param['index_l'][g.VALUE]).display_name, gr_index_l * 100))
        log.info("当前%s指数的20日涨幅 [%.2f%%]" % (
            self.get_security_info(g.param['index_s'][g.VALUE]).display_name, gr_index_s * 100))
        ...
        if gr_index_l <= g.param['index_growth_rate'][g.VALUE] and gr_index_s <= g.param['index_growth_rate'][g.VALUE]:
            ...
            if g.cache['day_count'] % g.param['period'][g.VALUE] == 0:
                ...
                self.position_adjust(context, buy_stocks)
            g.cache['day_count'] += 1
    ...
    def set_filter(self):
        ...
        self.func_register(g.filter, self.filter_by_query, '查询财务数据库过滤')
        self.func_register(g.filter, self.filter_gem, '过滤创业版股票')
        self.func_register(g.filter, self.filter_paused, '过滤停牌股票')
        self.func_register(g.filter, self.filter_st, '过滤ST及其他具有退市标签的股票')
        self.func_register(g.filter, self.filter_limitup, '过滤涨停的股票')
        self.func_register(g.filter, self.filter_limitdown, '过滤跌停的股票')
        self.func_register(g.filter, self.filter_blacklist, '过滤黑名单股票')
        # func_register(g.filter,filter_by_growth_rate, '过滤n日增长率为负的股票')
        self.func_register(g.filter, self.filter_new, '过滤新股')
        ...
        self.func_register(g.filter, self.filter_by_rank, '评分过滤器')
    ...
    def set_stop_loss(self):
        ...
        self.func_register(g.stop_loss_minute, self.stop_loss_by_price, '大盘价格止损')
        self.func_register(g.stop_loss_minute, self.stop_loss_by_3_crows, '三黑鸦止损')
        self.func_register(g.stop_loss_minute, self.stop_loss_by_index_l, '二八止损')
        self.func_register(g.stop_loss_minute, self.stop_loss_by_stock, '个股止损')
        self.func_register(g.stop_loss_minute, self.stop_profit_by_stock, '个股止盈')
    ...
    def filter_paused(self,stock_list, context, data):
        '''
        过滤停牌股票
        '''
    ...
    def filter_st(self,stock_list, context, data):
        '''
        过滤ST及其他具有退市标签的股票
        '''
    ...
    def filter_gem(self,stock_list, context, data):
        '''
        过滤创业版股票
        '''
    ...
    def filter_limitup(self,stock_list, context, data):
        '''
        过滤涨停的股票
        '''
    ...
    def filter_limitdown(self,stock_list, context, data):
        '''
        过滤跌停的股票
        '''
    ...
    def filter_by_growth_rate(self,stock_list, context, data):
        '''
        过滤n日增长率为负的股票
        '''
        n = 20
    ...
    def filter_blacklist(self,stock_list, context, data):
        '''
        过滤黑名单股票
        '''
    ...
    def filter_by_rank(self,stock_list, context, data):
        '''
        评分过滤器
        '''
    ...
    # 来自最大回撤抄底发
    def filter_by_chaodie(self,stock_list, context, data):
        ...
        for stock in stocks :
            ...
            value_buy_macd = self.buy_macd(stock[0])
            if value_buy_macd > 0:
    ...
    # #来自基金B二版
    def delect_stock(self,stock_list, context, data):
    ...
    def filter_market_time(self,stock_list, context, data):
        # 先计算周线
        log.info("==> 判断MACD止损")
        ...
        wDif, wDea, wMacd = self.MACD_CN(close, 12, 26, 9)
        ...
        if wMacd[-1] <= 0:
            ...
            close = hData['close']
            close = np.array(close, dtype='f8')
            dDif, dDea, dMacd = self.MACD_CN(close, 12, 26, 9)
            if dMacd[-1] <= 0:
                stock_list = []
        log.info("==> 结束MACD止损")
        return stock_list
    ...
    def stop_loss_by_3_crows(self,context, data):
        '''
    ...
    def stop_loss_by_index_l(self,context, data):
        '''
    ...
    def stop_loss_by_stock(self,context, data):
        '''
        个股止损
        minute 级别
        '''
        if g.param['is_stock_stop_loss'][g.VALUE]:
            ...
            for stock in context.portfolio.positions.keys():
                ...
                threshold = self._get_stop_loss_threshold(stock, g.param['period'][g.VALUE])
    ...
    def stop_profit_by_stock(self,context, data):
        '''
        个股止盈
        minute 级别
        '''
    ...
    def position_open(self,security, value):
        '''
                        开仓，买入指定价值的证券
    ...
    def position_adjust(self,context, buy_stocks):
        '''
        根据待买股票创建或调整仓位
