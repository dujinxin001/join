...
def set_param():
    ...
    if g.is_rank_stock:
        # 参与评分的股票数目
        g.rank_stock_count = 50

    # 买入股票数目
    g.buy_stock_count = 2
    
    # 配置二八指数
    #g.index2 = '399300.XSHE'  # 沪深300指数，表示二，大盘股
    g.index8 = '399905.XSHE'  # 中证500指数，表示八，小盘股
    ...
    if g.is_market_stop_loss_by_price:
        ...
        g.index_4_stop_loss_by_price = '399300.XSHE'

    # 配置三黑鸦判定指数，默认为上证指数，可修改为其他指数
    g.index_4_stop_loss_by_3_black_crows = '399300.XSHE'
...
def handle_data(context, data):
    ...
    if g.is_stock_stop_loss:
        ...
        stock_stop_loss_macd(context, data)
...
def do_handle_data(context, data):
    ...
    if g.day_count % g.period == 0:
        log.info("==> 满足条件进行调仓")
        #执行选股
        buy_stocks = pick_stocks(context, data,gr_index2,gr_index8)
        if len(buy_stocks)==0:
            log.info("选股后无买股票: %s" %(buy_stocks))
            clear_position(context)
            g.day_count=0
            return
        log.info("选股后有可买股票: %s" %(buy_stocks))
...
def market_stop_loss_by_price(context, index):
    ...
    if not g.is_day_stop_loss_by_price:
        ...
        if high_price > 2.2 * low_price \
            ...
            # 当日第一次输出日志
            log.info("==> 大盘止损，%s指数前160日内最高价超过最低价2.2倍, 最高价: %f, 最低价: %f" %(get_security_info(index).display_name, high_price, low_price))
...
def market_stop_loss_by_28_index(context, count):
    ...
    if gr_index2 <= g.index_growth_rate and gr_index8 <= g.index_growth_rate:
        if (g.minute_count_28index_drop == 0):
            log.info("当前二八指数的20日涨幅同时低于[%.2f%%], %s指数: [%.2f%%], %s指数: [%.2f%%]" %(g.index_growth_rate*100, get_security_info(g.index2).display_name, gr_index2*100, get_security_info(g.index8).display_name, gr_index8*100))
...
def filter_limitup_stock(context, stock_list):
    last_prices = history(2, unit='1m', field='close', security_list=stock_list)
    current_data = get_current_data()
...
def filter_limitdown_stock(context, stock_list):
    ...
    log.info("last_prices:%s"%last_prices)
    current_data = get_current_data()
...
'''
# 过滤20日线和macd
def filter_20ma_macd(data, stock_list):
    log.info("20日线和macd过滤前的股票%s" % (stock_list))
    stock_list2=[]
    for stock in stock_list:
        h = attribute_history(stock, 130, unit='5d', fields=('close', 'high', 'low'), skip_paused=True)
        #wh = attribute_history(stock, 130, unit='5d', fields=('close', 'high', 'low'), skip_paused=True)

        #cur_price = data[stock].close
        #avg_20 = data[stock].mavg(20, field='close')
        #log.info("%s的当前价:%s,20日均价%s" % (stock,cur_price,avg_20))
        #if cur_price<avg_20:
        #   log.info("被20日线过滤的股票 %s" % (stock))
        ...
        #dDif, dDea, dMacd =  MACD_CN(close, 12, 26, 9)
        #wclose = wh['close']
        #wclose = np.array(wclose, dtype='f8')
        #wDif, wDea, wMacd =  MACD_CN(wclose, 12, 26, 9)
        #log.info("%s的当前价dif:%s,dDea%s,macd%s,周macd%s" % (stock,dDif[-1],dDea[-1],dMacd[-1],wMacd[-1]))
        rsiValue1 = RSI_CN(close, 6)
        rsiValue2 = RSI_CN(close, 12)
        rsiValue3 = RSI_CN(close, 24)
        log.info("%s的当前价rsiValue1:%s,rsiValue2%s,rsiValue3%s" % (stock,rsiValue1[-1],rsiValue2[-1],rsiValue3[-1]))
        #if dDif[-1]>=dDea[-1] and cur_price>avg_20 and close[-1]<=avg_20:
        if rsiValue1[-1]>=rsiValue2[-1] and rsiValue2[-1]>rsiValue3[-1] and rsiValue3[-1]>=50 and rsiValue3[-1]>rsiValue3[-2]:
            log.info("符合macd条件的股票 %s" % (stock))
            stock_list2.append(stock)
            continue
    log.info("20日线和macd过滤后的股票%s" % (stock_list2))
    return stock_list2


# 选股
# 选取指定数目的小市值股票，再进行过滤，最终挑选指定可买数目的股票
def pick_stocks(context, data,gr_index2,gr_index8):
    #查询股票市值数据
    log.debug("valuation.code %s" % (valuation.code))
    ...
    #按市值大小排序（pick_stock_count:备选股票数目
    log.debug("市值排序前q= %s" % (q))
    q = q.order_by(
                valuation.market_cap.asc()
            ).limit(
                g.pick_stock_count
            )
    log.debug("市值排序后q= %s" % (q))
    #查询股票财务数据
    df = get_fundamentals(q)
    #取出股票代码
    stock_list = list(df['code'])
    #stock_list = ['510300.XSHG','510500.XSHG']
    #stock_list = ['150052.XSHE','150153.XSHE']
    #stock_list = ['002687.XSHE']
    ...
    #stock_list = filter_by_growth_rate(stock_list, 20)
     #过滤20日线和macd
    stock_list=filter_20ma_macd(data,stock_list)
    #对股票进行评分（rank_stock_count：参与评分的数量）
    if g.is_rank_stock:
